...
// the key is what makes a dictionary a dictionary
...
// mapped type
// only known properties allowed. This is now a record not a dictionary
...
// builtin TypeScript type (Record)
type Record<K extends keyof any, T> = {
  [P in K]: T
}

const myFruit: Record<'first' | 'second', Fruit>
// myFruit.first and second both show for myFruit
...
type DataSDK = {
  // The mapped type
  // iterate over all of the keys and transform each key and capitalize K
...
// using infer and will capture whatever is before Masters
type ExtractMasterName<S> = S extends `${infer T extends
  `F${any}`} Masters`
  ? T
  : never
